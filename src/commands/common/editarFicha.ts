import { ApplicationCommandOptionType, ApplicationCommandType, CategoryChannel, ChannelType, Collection, MessageCreateOptions, TextChannel } from "discord.js";
import { getCharacter, getEditCharacterId, setEditCharacterId, updateCharacter } from "../../helpers/dbService";
...
export default new Command({
    ...
    options: [{
        ...
        options: [{
            ...
            name: "pt2",
            type: ApplicationCommandOptionType.Subcommand,
            description: "Editar PV, Creditos, Perolas, Foto, Cor",
            options: [{
                name: "usuario",
                description: "Usuario a ser editado",
                type: ApplicationCommandOptionType.User,
    ...
    modals: new Collection([
        ["form-ficha", async (modalInteraction) => {
            try {
        ...
        ["form-dm-edit", async (modalInteraction) => {
            try {
                const { fields } = modalInteraction;
        ...
        ["form-edit-aprendizados", async (modalInteraction) => {
            try {
                const { fields } = modalInteraction;
                const characterId = await getEditCharacterId(modalInteraction.user.id);
